...
cloudinary.config({cloud_name: 'dcronwamq', api_key: '597153926796645', api_secret: 'gAET1_v4TT3W4eNwVBGqE78_NzU'});
...
router.put('/join-to-event', (req, res) => {
    ...
    Event.find({
        ...
        if (err) {
            ...
            if (toggle) {
                if (joinedPlayersIndex === -1 && eventPlayers.count.current<eventPlayers.count.max
          ) {
            eventPlayers.joined.push(userId);
            eventPlayers.count.current = Math.min(
              eventPlayers.count.current + 1, eventPlayers.count.max
            );
          }
        } else {
          if (joinedPlayersIndex> -1) {
                    eventPlayers.joined.splice(joinedPlayersIndex, 1);
                    eventPlayers.count.current = Math.max(eventPlayers.count.current - 1, 0);
                }
            }
...
router.post('/edit-user/:userId', upload.single('photo'), (req, res) => {
    ...
    for (let prop in params.personal) 
        if (! params.personal[prop]) 
            delete params.personal[prop];
        
    

    for (let prop in params.personal.location) 
        if (! params.personal.location[prop]) 
            delete params.personal.location[prop];
    ...
    User.update({
        _id: userId
    }, convertToDotNotation(params), function (err) {
        if (err) {
            sendError(err, res);
        } else {
